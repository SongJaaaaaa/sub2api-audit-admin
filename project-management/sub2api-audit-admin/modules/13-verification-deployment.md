...
| 项 | 状态 | 备注 |
|---|---|---|
| 后端全量测试 | 已完成 | 55 passed、349 assertions |
...
| 次日自动对账 | 待生产执行 | 验证 `00:15 Asia/Shanghai` Scheduler |
...
6. 安装每分钟 `php artisan schedule:run`，确认每天 `00:15 Asia/Shanghai` 自动对账。
...
必须保留历史可查，但不得进入切账后当前财务、告警和对账。不得在代码、测试或文档中硬编码生产远端事件 ID。
...
- 状态只有 `ok | warning | error`。
- 只有外部事件或审计孤儿时允许 `warning`，不强行要求问题数量为 0 才算任务完成。
- 同日手动补跑替换原批次和差异明细，不返回旧版重复对账 `409`。
