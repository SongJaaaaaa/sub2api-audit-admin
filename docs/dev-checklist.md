...
- 切账日账务和对账从精确切账时间开始，不从当天 00:00 开始。
- 官方用量不受切账时间截断。
...
- 状态仅为 `ok`、`warning`、`error`。
- 只有外部事件或审计孤儿时为 `warning`；本地缺失或字段不一致时为 `error`。
- 同一业务日期手动补跑会重新计算并替换原差异明细，不返回旧版 409，也不重复累加。
- 切账前日期拒绝生成当前对账。
- `php artisan ledger:reconcile` 默认处理上一中国自然日。
- Scheduler 每天 `00:15 Asia/Shanghai` 调用同一服务，并启用 `withoutOverlapping`。
...
- 模型统计、Sub2API 用户、调额记录、现金/赠送账、经营账、历史账、对账中心、异常中心、操作审计可打开。
- 调额提交前有明确确认文案。
