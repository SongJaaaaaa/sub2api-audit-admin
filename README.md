...
用于安全调整 Sub2API 用户余额、记录本地审计台账，并按统一口径查看财务、用量、余额、排行、历史余额事件和真实对账结果。
...
| 数据源 | 用途 |
|---|---|
| 本地审计 SQLite | 本地现金/赠送、调增/调减、现金入账用户排行、调额记录、对账批次和操作审计 |
| Sub2API 官方 Admin API | 请求数、四类 Token、标准消费、实际消费、用户排行和 requested 模型统计 |
| Sub2API PostgreSQL 只读连接 | 用户累计充值快照、普通启用用户当前余额、后台调额事件关联、历史余额事件和真实对账 |
